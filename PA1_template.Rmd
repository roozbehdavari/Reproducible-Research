...
On average across all the days in the dataset, the interval `r max.steps` contains the maximum number of steps. Assuming the first measurments is started at 12:00 PM, the `r max.steps`th interval has started at `r max.steps.formatted`.
...
For imputed missing value dataset, the mean of total number of steps taken per day is `r mean.steps` while the median is `r median.steps`. Note that both mean and median have changed for only few percents. This was expected as we imputed the missing values based on random replacement of existing values. 
